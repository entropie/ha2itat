...
module Plugins
  ...
  module Booking
    ...
    class Events < Array
      ...
      class Event
        ...
        def self.inherited(o)
          ...
          Ha2itat.log "adding #{o} to EventTypes"
        end
        ...
        def self.create(paramhash)
          ...
          normalized_params[:ident] = ::Ha2itat::Database::make_slug(normalized_params[:ident])
          if existing = Ha2itat.adapter(:booking).events.by_slug(normalized_params[:ident])
        ...
        def self.exist?(slug)
          Ha2itat.adapter(:booking).events.by_slug(slug)
        end
        ...
        def ident_suggestion
          ...
          args = [is, Ha2itat::Database.get_random_id(6), start_date.strftime("%U"), start_date.strftime("%y")]
          default_str % args
        end
        ...
        def attend(att_hash, slot = nil, &blk)
          ...
          @attender = nil
          Ha2itat::Mixins::FU.write(attn.filename, YAML::dump(attn))
        ...
        def attender_path(*args)
          Ha2itat.adapter(:booking).repository_path("eventattender", slug, *args)
        end
        ...
        def exist?
          ::File.exist?(Ha2itat.adapter(:booking).repository_path(filename))
        end
        ...
        def datapath(*args)
          Ha2itat.quart.media_path(relative_datapath, *args)
        end
      ...
      class Agenda < Hash
        ...
        def transform(events)
          merge_proc = lambda {|ev, identdate|
            (self[identdate] ||= Events.new(Ha2itat.adapter(:booking))) << ev.dup.for_date(identdate)
          }
          events.each do |ev|
            if ev.recurrent?
    ...
    class ArchivedEvents < Events
      ...
      def self.path(*args)
        adapter_path = Ha2itat.adapter(:booking).repository_path( relative_path )
